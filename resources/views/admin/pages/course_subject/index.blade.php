...
@section('title', 'List Course and Subject')
@section('content')
<style>
...
<section class="content-header">
    <h1>
        @yield('title')
        <small><a href="{{route('course-subject.create')}}" class="btn btn-success btn-xs btn-flat btn-sm" title="Create Department"><i class="fa fa-plus"></i> CREATE</a></small>
    </h1>
</section>
                    ...
                    <div class="table-responsive">
                        ...
                        <table id="example1" class="table table-striped table-bordered border gy-4 gs-4">
                            ...
                            <tbody>
                                <?php
                                    $i=1;
                                    $datas = DB::table('subjects')->get();
                                    foreach ($datas as $data) {
                                    ?>
                                <tr>
                                    <td style="text-align: center;">{{$i}}</td>
                                    <td style="text-align: center;">{{ $data->title }}</td>
                                    <td style="text-align: center;">{{ $data->subject_name }}</td>
                                    <td>
                                        <a title="Edit Pages" href="{{route('course-subject.edit',$data->id)}}" class="btn btn-icon btn-circle btn-sm btn-warning"><i class="fa fa-pencil-alt"></i></a>
                                    </td>
                                </tr>
                                <?php
                                $i++;
