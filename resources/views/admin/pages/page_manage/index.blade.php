...
<section class="content-header">
    <h1>
        @yield('title')
        <small><a href="{{route('pages.create')}}" class="btn btn-success btn-xs btn-flat btn-sm" title="Create Department"><i class="fa fa-plus"></i> CREATE</a></small>
    </h1>
</section>
                    ...
                    <div class="table-responsive">
                        ...
                        <table id="example1" class="table table-striped table-bordered border gy-4 gs-4">
                            ...
                            <tbody>
                                <?php
                                    $i=1;
                                    $datas = DB::table('page_manages')->get();
                                    foreach ($datas as $data) {
                                    ?>
                                <tr>
                                    <td style="text-align: center;">{{$i}}</td>
                                    <td style="text-align: center;">{{ $data->page_name }}</td>
                                    @if (!empty($data->image))
                                    <td style="text-align: center;"> <img src="{{url('public/images/page-image/'.$data->image)}}" alt="Image" style="width: 50px; height: 50px;"> </td>
                                    @else
                                    <td> <img src="{{ url('public/images/page-image/no-image.png')}}" style="width: 50px; height: 50px;" alt="image" style="object-fit: contain;" /></td>
                                    @endif
                                    <td style="text-align: center;"> <img src="{{url('public/images/inner-banner/'.$data->banner_img)}}" alt="Image" style="width: 100px; height: 50px;"> </td>
                                        ...
                                        <div class="tab-pane active" id="kt_builder_toolbar">
                                            <div class="row">
                                                <div class="col-lg-4 col-xl-6">
                                                    <form action="{{route('pages.pagesstatuschanger','id='.$data->id) }}" name="sstc{{$data->id}}" id="sstc{{$data->id}}" method="post">
                                                        {{ csrf_field() }}
                                    ...
                                    <td>
                                        <a title="Edit Pages" href="{{route('pages.edit',$data->id)}}" class="btn btn-icon btn-circle btn-sm btn-warning"><i class="fa fa-pencil-alt"></i></a>
                                    </td>
                                </tr>
                                <?php
                                $i++;
