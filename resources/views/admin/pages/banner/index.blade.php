...
<section class="content-header">
    <h1>
        @yield('title')
      <small><a href="{{route('banner.create')}}" class="btn btn-success btn-xs btn-flat btn-sm" title="Create Banner"><i class="fa fa-plus"></i> CREATE</a></small>
    </h1>
</section>
...
<div class="box">
    ...
    <div class="box-body">
            ...
            <table id="example1" class="table table-striped table-bordered border gy-4 gs-4">
                <thead>
                    <tr class="fw-bold fs-6">
                        ...
                        <th style="text-align: center;">Banner Caption</th>
        ...
        <?php
            $i=1;
            $datas = DB::table('banners')->get();
            foreach ($datas as $data) {
            ?>
        <tr>
            <td style="text-align: center;">{{$i}}</td>
            <td style="text-align: center;">{{ $data->banner_title }}</td>
            <td style="text-align: center;"> <img src="{{url('public/images/banner/'.$data->banner_img)}}" alt="Image" style="width: 50px; height: 50px;"> </td>

            <td>
                <div class="tab-pane active" id="kt_builder_toolbar">
                  <div class="row">
                    <div class="col-lg-4 col-xl-6">
                     <form action="{{route('banner.bannerstatuschanger','id='.$data->id) }}" name="sstc{{$data->id}}" id="sstc{{$data->id}}" method="post">
                      {{ csrf_field() }}
            ...
            <td >
                <a title="Edit Banner" href="{{route('banner.edit',$data->id)}}" class="btn btn-icon btn-circle btn-sm btn-warning"><i class="fa fa-pencil-alt"></i></a>
            </td>
        </tr>
        <?php
    $i++;
