...
<section class="content-header">
    <h1>
        @yield('title')
        <small><a href="{{route('notice.create')}}" class="btn btn-success btn-xs btn-flat btn-sm" title="Create Department"><i class="fa fa-plus"></i> CREATE</a></small>
    </h1>
</section>
                    ...
                    <div class="table-responsive">
                        ...
                        <table id="example1" class="table table-striped table-bordered border gy-4 gs-4">
                            ...
                            <tbody>
                                <?php
                                    $i=1;
                                    $datas = DB::table('notices')->get();
                                    foreach ($datas as $data) {
                                    ?>
                                <tr>
                                    <td style="text-align: center;">{{$i}}</td>
                                    <td style="text-align: center;">{{ $data->title }}</td>
                                    <td style="text-align: center;">{{ (date('d-m-Y',strtotime($data->date_time))) }}</td>
                                    <td style="text-align: center;"><?php echo (substr(strip_tags($data->description),0,400)) ?></td>
                                    <td style="text-align: center;"><a href="{{url('public/images/notice/'.$data->pdf_file)}}" target="_blank" download><i style="font-size:26px; color:#0029f1;" class="fa fa-download" aria-hidden="true"></i></a> </td>
                                    <td>
                                        <div class="tab-pane active" id="kt_builder_toolbar">
                                            <div class="row">
                                                <div class="col-lg-4 col-xl-6">
                                                    <form action="{{route('notice.noticestatuschanger','id='.$data->id) }}" name="sstc{{$data->id}}" id="sstc{{$data->id}}" method="post">
                                                        {{ csrf_field() }}
                                    ...
                                    <td>
                                        <a title="Edit Notice" href="{{route('notice.edit',$data->id)}}" class="btn btn-icon btn-circle btn-sm btn-warning"><i class="fa fa-pencil-alt"></i></a>
                                    </td>
                                </tr>
                                <?php
                                $i++;
